...
def cmd_parser():
    ...
    parser.add_option("-f", "--frames",     help="Number of frames to train on per gesture", default=300, type="int")
    parser.add_option("-w", "--words",      help="Number of visual words", default=200, type="int")#change it 30,50,100....
    ...
    parser.add_option("-k", "--kernel",     help="Kernel type", action="store", type="string", default="linear")#linear,hist,nural,tree,sgd,forest,bagging,gradientBoost,voting
...
if __name__ == "__main__":
    ...
    try:
        if inputMode == "video":
            ...
            print "Training score video= {0}".format(score)
            ...
            print "Training score descriptors = {0}".format(score)
            ...
            print "Training score images= {0}".format(score)
            outTrainDir = get_new_directory(opts.num, opts.type)
            clf = pickle_files(outTrainDir, recognizer.trainer)
        else:
            recognizer = Recognizer(vc=vc, opts=opts)
